...
cols=['myy','y_pred','y']
data=pd.read_csv('ANN_results.csv',names=cols)
sig=data[data['y']==1]
bkg=data[data['y']==0]
...
plt.hist(sig['y_pred'], bins, color='orange',
         histtype='step',
...
# background
plt.figure()
bins = np.linspace(105,160,65)

high_disc_bkg=bkg[bkg['y_pred']>=0.5]

plt.hist(high_disc_bkg['myy'], bins, color='red',
         histtype='step',
         label=r'background, D>0.5',
