...
#------------------------------------------------------------------------
# 2) Detour: Classification with transformed variables  
print('\n ================== 2) Scaled classifier  ================ \n ')

# scale: pt/myy and E/myy 
# Import scaled data
data_scaled=pd.read_csv('data_scaled_2M.csv')
print('Imported scaled data:')
print(data_scaled.head(4))

myy_sc = data_scaled['myy'].values
y_sc = data_scaled['label'].values
X_sc = data_scaled[['pt_y1','eta_y1','phi_y1','e_y1',
                    'pt_y2','eta_y2','phi_y2','e_y2']].values

# Normalize X data
normalize = StandardScaler()
X_sc = normalize.fit_transform(X_sc)
      
# Split data into training and testing sets
X_sc_train, X_sc_test, y_sc_train, y_sc_test, myy_sc_train, myy_sc_test = train_test_split(
    X_sc, y_sc, myy_sc, test_size = 0.30, random_state = 42)

clf_scaled = classifier(num_feat)
clf_scaled.compile(optimizer='adam', loss='binary_crossentropy')
hist_clf_scaled = clf_scaled.fit(X_sc_train, y_sc_train,
                                 epochs = num_epochs, batch_size = batch,
                                 validation_split = 0.2, verbose = 2)

# generate predictions & store results 
y_pred = clf_scaled.predict(X_sc_test).flatten()
res = np.array([myy_sc_test.T, y_pred, y_sc_test])
np.savetxt('clf_scaled_results.csv', res.T, delimiter = ',')

# store training history 
with open('clf_scaled_history.pckl', 'wb') as file_pi:
    pickle.dump(hist_clf_standalone.history, file_pi)
