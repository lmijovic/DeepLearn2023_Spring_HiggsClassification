...
# Import data
data = np.loadtxt('features-scaled.csv', delimiter = ',')
X = data[:,:n+1]
y = data[:,n+1]
...
plot_model(model, to_file='scaled-NN-architecture.png', show_shapes=True, show_layer_names=True)
...
# Training process
hist = model.fit(X_train[:,1:], y_train, epochs=nep, batch_size=5000, validation_split = 0.2, callbacks = [stop])
...
atlasify('work in progress', enlarge = 1)
plt.show()

# Write outfile
myy = normalize.inverse_transform(X_test)[:,0]
res = np.array([myy.T, y_pred, y_test])
np.savetxt('discriminant-scaled.csv', res.T, delimiter = ',')
